...
`object-fit` -> Especifica como um elemento (normalmente imagem) deve ser ajustado a uma caixa. Contém as propriedades `fill`, `contain`, `cover`, `scale-down` e `none`.
...
`z-index` -> Funciona como as camadas no Photoshop, o número padrão é 0, e quanto maior o número, mais "por cima" esse elemento ficará dos outros.
